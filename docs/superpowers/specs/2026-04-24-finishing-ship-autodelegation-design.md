...
- `## Summary` — 2–3 bullets derived from commits since base branch (subjects only, deduplicated by conventional-commit type)
...
Action: announce → invoke `ship-rebase-conflict-resolution` via the Skill tool → on its return, retry `git push` exactly once. If the retry fails, fatal: report, stop, operator resolves manually.
