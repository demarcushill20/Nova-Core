...
class TestScalingPlanIntegration:
    ...
    def test_scaling_calendar_cycle_initialization_on_first_trade(self):
        ...
        try:
            ...
            from novatrade.risk.profit_cushion_protocol import ProfitCushionProtocol

            with patch('novatrade.risk.scaling_plan_calendar.ScalingPlanConfig') as mock_scaling_config:
                with patch('novatrade.risk.profit_cushion_protocol.ProfitCushionConfig') as mock_profit_config:
                    ...
                    profit_config.get_risk_multiplier = lambda profit_pct: (
                        ("scaling_lock", 0.10) if profit_pct >= 10.0 else
                        ("turtle", 0.25) if profit_pct >= 8.0 else
                        ("selective", 0.5) if profit_pct >= 5.0 else
                        ("conservative", 0.75) if profit_pct >= 3.0 else
                        ("normal", 1.0)
                    )
    ...
    def test_scaling_calendar_error_handling(self):
        ...
        try:
            ...
            with patch('novatrade.risk.pre_trade_gate.ScalingPlanCalendar', side_effect=Exception("Calendar init failed")):
                gate = PreTradeGate(self.cfg)
