...
class TestHighRiskCodingRejected:
    ...
    def test_secret_handling_rejected(self):
        ...
        eligible, reason = is_stageC_eligible("code_impl", 0.8, task, _stageC_flags())
...
class TestResearchUnderStageC:
    ...
    def test_research_task_still_eligible(self):
        ...
        eligible, reason = is_stageC_eligible("research", 0.8, task, _stageC_flags())
        assert eligible is True
...
class TestIneligibleClassStageC:
    ...
    def test_unknown_class_rejected(self):
        eligible, reason = is_stageC_eligible("unknown", 0.0, "xyzzy", _stageC_flags())
        assert eligible is False
...
class TestDisabledFlagStageC:
    ...
    def test_coding_rejected_when_disabled(self):
        ...
        eligible, reason = is_stageC_eligible("code_impl", 0.9, task, _disabled_flags())
        assert eligible is False
        assert reason == "orchestrator_disabled"

    def test_research_rejected_when_disabled(self):
        task = "Research the latest developments in quantum computing"
        eligible, reason = is_stageC_eligible("research", 0.9, task, _disabled_flags())
        assert eligible is False
        assert reason == "orchestrator_disabled"
...
class TestConfidenceStageC:
    ...
    def test_low_confidence_coding_rejected(self):
        eligible, reason = is_stageC_eligible("code_impl", 0.3, "Refactor task", _stageC_flags(min_confidence=0.5))
        assert eligible is False
        assert "below" in reason

    def test_exact_threshold_accepted(self):
        eligible, reason = is_stageC_eligible("code_impl", 0.5, "Refactor task", _stageC_flags(min_confidence=0.5))
        assert eligible is True

    def test_above_threshold_accepted(self):
        eligible, reason = is_stageC_eligible("code_impl", 0.9, "Refactor task", _stageC_flags(min_confidence=0.5))
        assert eligible is True

    def test_zero_confidence_rejected(self):
        eligible, reason = is_stageC_eligible("code_impl", 0.0, "Refactor task", _stageC_flags())
        assert eligible is False
...
class TestStageCPlanValidation:
    ...
    def test_coding_plan_valid(self):
        plan = build_plan_from_task("test_task", "Refactor the module to reduce duplication", routing={"stage": "C"})
        valid, reason = validate_stageC_plan(plan)
        assert valid is True
        assert "verifier_present" in reason

    def test_code_review_plan_valid(self):
        plan = build_plan_from_task("test_task", "Review code changes for quality issues", routing={"stage": "C"})
        valid, reason = validate_stageC_plan(plan)
        assert valid is True

    def test_research_plan_under_stageC_valid(self):
        plan = build_plan_from_task("test_task", "Research AI safety approaches and summarize", routing={"stage": "C"})
        valid, reason = validate_stageC_plan(plan)
        assert valid is True
    ...
    def test_coding_plan_strategy_name(self):
        plan = build_plan_from_task("test_task", "Refactor the module", routing={"stage": "C"})
        assert plan.strategy == "stageC_coding"

    def test_research_plan_under_stageC_strategy(self):
        plan = build_plan_from_task("test_task", "Research AI safety and summarize approaches", routing={"stage": "C"})
...
class TestVerifierEnforcement:
    ...
    def test_stageC_coding_plan_always_has_verifier_step(self):
        plan = build_plan_from_task("test_code", "Refactor the validation module", routing={"stage": "C"})
    ...
    def test_stageC_code_review_plan_has_verifier_step(self):
        plan = build_plan_from_task("test_review", "Review code for quality issues in the API", routing={"stage": "C"})
        skill_names = [s.skill_name for s in plan.steps]
        assert "self-verification" in skill_names
...
class TestStageCFallback:
    ...
    def test_nonsense_task_falls_back(self):
        ...
        assert cls == "unknown"
        eligible, reason = is_stageC_eligible(cls, conf, "xyzzy plugh qwerty", _stageC_flags())
        assert eligible is False
    ...
    def test_ambiguous_with_low_confidence_falls_back(self):
        ...
        cls, conf = classify_task(task)
        eligible, reason = is_stageC_eligible(cls, conf, task, _stageC_flags(min_confidence=0.8))
        assert eligible is False
...
class TestFailClosedDefaults:
    ...
    def test_missing_stage_treated_as_not_C(self):
        ...
        eligible, reason = is_stageC_eligible("code_impl", 0.9, "Refactor task", flags)
        assert eligible is False
    ...
    def test_stageB_flag_rejects_stageC_check(self):
        ...
        eligible, reason = is_stageC_eligible("code_impl", 0.9, "Refactor task", _stageB_flags())
        assert eligible is False
        assert "not_C" in reason
...
class TestStageBBackwardCompat:
    ...
    def test_stageB_rejects_coding_with_B_flag(self):
        ...
        eligible, reason = is_stageB_eligible("code_impl", 0.8, task, _stageB_flags())
        assert eligible is False
