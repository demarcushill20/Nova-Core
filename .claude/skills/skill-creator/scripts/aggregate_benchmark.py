...
def generate_markdown(benchmark: dict) -> str:
    ...
    delta_pr = delta.get("pass_rate", dash)
    lines.append(
        f"| Pass Rate | {a_pr.get('mean', 0) * 100:.0f}% {pm} {a_pr.get('stddev', 0) * 100:.0f}% | {b_pr.get('mean', 0) * 100:.0f}% {pm} {b_pr.get('stddev', 0) * 100:.0f}% | {delta_pr} |"
    )

    # Format time
    a_time = a_summary.get("time_seconds", {})
    b_time = b_summary.get("time_seconds", {})
    delta_time = delta.get("time_seconds", dash)
    lines.append(
        f"| Time | {a_time.get('mean', 0):.1f}s {pm} {a_time.get('stddev', 0):.1f}s | {b_time.get('mean', 0):.1f}s {pm} {b_time.get('stddev', 0):.1f}s | {delta_time}s |"
    )

    # Format tokens
    a_tokens = a_summary.get("tokens", {})
    b_tokens = b_summary.get("tokens", {})
    delta_tokens = delta.get("tokens", dash)
    lines.append(
        f"| Tokens | {a_tokens.get('mean', 0):.0f} {pm} {a_tokens.get('stddev', 0):.0f} | {b_tokens.get('mean', 0):.0f} {pm} {b_tokens.get('stddev', 0):.0f} | {delta_tokens} |"
    )
