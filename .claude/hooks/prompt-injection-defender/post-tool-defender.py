...
def load_config() -> dict[str, Any]:
    ...
    if project_dir:
        project_config = Path(project_dir) / ".claude" / "hooks" / "prompt-injection-defender" / "patterns.yaml"
...
def extract_text_content(tool_name: str, tool_result: Any) -> str:
    ...
    if isinstance(tool_result, dict):
        ...
        if "file" in tool_result and isinstance(tool_result["file"], dict) and "content" in tool_result["file"]:
...
def scan_for_injections(text: str, config: dict[str, Any]) -> list[tuple[str, str, str, str]]:
...
def format_warning(detections: list[tuple[str, str, str, str]], tool_name: str, source_info: str) -> str:
    ...
    if high_severity:
        ...
        for category, _pattern, reason, _severity in high_severity:
            lines.append(f"  - [{category}] {reason}")
        lines.append("")

    if medium_severity:
        lines.append("MEDIUM SEVERITY DETECTIONS:")
        for category, _pattern, reason, _severity in medium_severity:
            lines.append(f"  - [{category}] {reason}")
        lines.append("")

    if low_severity:
        lines.append("LOW SEVERITY DETECTIONS:")
        for category, _pattern, reason, _severity in low_severity:
            lines.append(f"  - [{category}] {reason}")
        lines.append("")
