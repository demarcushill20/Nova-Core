...
class RateLimitingReconnectMixin:
    ...
    def _setup_guardian_listeners(self):
        ...
        def on_feed_staleness(staleness_seconds: float):
            log.warning("Feed staleness detected: %.1fs - monitoring for reconnection needs",
                       staleness_seconds)
    ...
    def _is_subscription_quota_error(self, exc: Exception) -> bool:
        ...
        return (
            exc_name == "TooManyRequestsException" and
            "subscription" in exc_str and
            "quota" in exc_str
        ) or (
            "account subscriptions quota" in exc_str or
            "subscriptions available" in exc_str
        )
    ...
    async def _auto_reconnect_with_rate_limiting(self) -> bool:
        ...
        try:
            for attempt in range(max_attempts):
                ...
                if not can_connect:
                    log.warning(
                        "metaapi rate-limited reconnect: attempt %d/%d blocked - %s",
                ...
                try:
                    ...
                    if status.connected:
                        ...
                        log.warning("metaapi rate-limited reconnect: attempt %d failed: %s",
                                  attempt + 1, status.message)
                    ...
                    if self._is_subscription_quota_error(exc):
                        ...
                        log.error("Subscription quota exhausted in reconnect attempt %d: %s",
                                attempt + 1, exc)
                    ...
                    if "429" in error_str or "rate limit" in error_str or "too many requests" in error_str:
                        error_code = 429
                        log.error("Rate limiting detected in reconnect attempt %d: %s",
                                attempt + 1, exc)
    ...
    async def _attempt_resubscribe_with_rate_limiting(self) -> bool:
        ...
        try:
            ...
            log.info("Feed resubscription successful for %d symbols",
                    len(getattr(self, '_subscribed_symbols', [])))
            return True
...
def enhance_adapter_with_rate_limiting(adapter_instance):
    ...
    def on_feed_staleness(staleness_seconds: float):
        log.warning("Feed staleness detected on adapter %s: %.1fs",
                   id(adapter_instance), staleness_seconds)
